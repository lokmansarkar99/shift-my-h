...
const POPULAR_STORES = [
  { name: 'IKEA Wembley', address: '2 Drury Way, London', postcode: 'NW10 0TH', lat: 51.5544, lng: -0.2608 },
  { name: 'IKEA Edinburgh', address: '95 Costkea Way, Loanhead', postcode: 'EH20 9BY', lat: 55.8778, lng: -3.1492 },
  { name: 'B&Q Edinburgh', address: 'Newcraighall Road, Edinburgh', postcode: 'EH15 3HS', lat: 55.9324, lng: -3.0845 },
  { name: 'IKEA Glasgow', address: '99 Kings Inch Drive, Glasgow', postcode: 'G51 4FB', lat: 55.8775, lng: -4.3644 },
  { name: 'B&Q London', address: 'Smugglers Way, Wandsworth, London', postcode: 'SW18 1EG', lat: 51.4608, lng: -0.1915 },
];

export function StoreStep1Address({ data, onChange, onNext }: StepProps) {
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 lg:p-8">
            ...
            <form onSubmit={handleNext} className="space-y-6">
              ...
              <div className="space-y-4">
                {/* Popular Stores chips */}
                <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4">
                  <div className="flex items-center gap-2 mb-3">
                    <Store className="w-4 h-4 text-emerald-600" />
                    <span className="text-xs font-bold text-slate-700 uppercase tracking-wider">Quick Select Popular Stores</span>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {POPULAR_STORES.map((store) => (
                      <button
                        ...
                        type="button"
                        onClick={() => {
                          setPickupAddress(store.address);
                          setPickupPostcode(store.postcode);
                          setPickupLat(store.lat);
                          setPickupLng(store.lng);
                        }}
                        className={`px-3 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 border-2 ${
                          pickupAddress === store.address
                            ...
                            : 'bg-white text-slate-600 border-slate-100 hover:border-emerald-300 hover:text-emerald-600'
                        }`}
                      >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    required
                    className="w-full px-4 py-3 rounded-xl border border-slate-300 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none bg-white"
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="flex items-center gap-3 cursor-pointer p-4 rounded-xl border border-slate-300 hover:border-emerald-400 hover:bg-emerald-50 transition-all">
                    <input
                      type="checkbox"
